...
void ProgressBarRound::paintEvent(QPaintEvent *)
{
    ...
    QRectF baseRect(10, 10, outerRadius - 20, outerRadius - 20);
...
void ProgressBarRound::drawValue(QPainter *painter, const QRectF &baseRect)
{
    ...
    QPen pen = painter->pen();
    pen.setColor(numberColor);
    pen.setWidth(dataPenWidth);
    pen.setCapStyle(Qt::RoundCap);
    painter->setPen(pen);

    if (barStyle == BarStyle_Line)//绘制线条风格
    {
...
void ProgressBarRound::drawText(QPainter *painter, const QRectF &innerRect, double innerRadius)
{
   ...
   // f.setPixelSize(innerRadius * qMax(0.05, (0.35 - (double)precision * 0.08)));
    ...
    QRectF numberRect(innerRect.x(), innerRect.y(), innerRect.width(), innerRect.height());
    QRectF textRect(innerRect.x(), innerRect.y() + innerRect.height() - m_height, innerRect.width(), m_height);
    //painter->setPen(palette().text().color());
    painter->drawText(numberRect, Qt::AlignCenter, valueToText(value));
...
void ProgressBarRound::setTextHeight(int height)
{
    m_height = height;
}

void ProgressBarRound::setNumberColor(const QColor &numberground)
{
    if(this->numberColor != numberground)
    {
        this->numberColor = numberground;
        update();
    }
}

void ProgressBarRound::setMyText(QString message)
{
    m_text = message;
}
